...
def failure_informed_sampling(model, A_func, candidate_p, epsilon_r=1e-3, epsilon_p=0.05, N_add=10, device='cpu'):
    model.eval()
    with torch.no_grad():
        residuals = []
        for p in candidate_p:
            ...
            pred = model(p_tensor)
            A_p = A_func(p).unsqueeze(0).to(device)
            I = torch.eye(A_p.shape[1], device=device).unsqueeze(0)
    ...
    fail_prob = len(fail_p) / len(candidate_p) if len(candidate_p) > 0 else 0.0
